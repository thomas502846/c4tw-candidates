---
name: kuraho-design
description: Design system skill for kuraho. Activate when building UI components, pages, or any visual elements. Provides exact color tokens, typography scale, spacing grid, component patterns, and craft rules. Read references/DESIGN.md before writing any CSS or JSX.
---

# kuraho Design System

You are building UI for **kuraho**. Dark-themed, warm palette, sans-serif typography (Noto Serif Japanese), compact density on a 4px grid.

## Visual Reference

**IMPORTANT**: Study ALL screenshots below before writing any UI. Match colors, typography, spacing, layout, and motion exactly as shown.

### Homepage

![kuraho Homepage](screenshots/homepage.png)

> Read `references/DESIGN.md` for full token details.

## Design Philosophy

- **Layered depth** — use shadow tokens to create a sense of physical layering. Each elevation level has a specific shadow.
- **Gradient accents** — gradients are used thoughtfully for emphasis, not decoration.
- **Type pairing** — Noto Serif Japanese for body/UI text, Raleway for headings/display. Never introduce a third typeface.
- **compact density** — 4px base grid. Every dimension is a multiple of 4.
- **warm palette** — the color temperature runs warm, matching the sans-serif typography.
- **Restrained accent** — `#cc0000` is the only pop of color. Used exclusively for CTAs, links, focus rings, and active states.
- **Subtle motion** — transitions smooth state changes. Keep durations under 300ms, use ease-out curves.

## Color System

### Core Palette

| Role | Token | Hex | Use |
|------|-------|-----|-----|
| Background | `--background` | `#333333` | Page/app background |
| Surface | `--surface` | `#000000` | Cards, panels, modals |
| Text Primary | `--text-primary` | `#ffffff` | Headings, body text |
| Text Muted | `--text-muted` | `#726660` | Captions, placeholders |
| Accent | `--accent` | `#cc0000` | CTAs, links, focus rings |

### Status Colors

| Status | Hex | Use |
|--------|-----|-----|
| Warning | `#ff8500` | Caution states, pending items |
| Danger | `#7a2c1d` | Errors, destructive actions |

### Extended Palette

- `#e3e0df` — Light surface or highlight color
- `#0066cc`
- `#cccccc`
- `#ffb90a`
- `#999999`
- `#7f001b`
- `#f1efef` — Light surface or highlight color
- `#990000`

## Typography

### Font Stack

- **Noto Serif Japanese** — Heading 1, Heading 2, Heading 3
- **Raleway** — Body, Caption

### Font Sources

```css
@font-face {
  font-family: "Noto Serif Japanese";
  src: url("fonts/NotoSerifJapanese-Regular.woff") format("woff");
  font-weight: 400;
}
@font-face {
  font-family: "Raleway";
  src: url("fonts/Raleway-Bold.ttf") format("truetype");
  font-weight: 700;
}
@font-face {
  font-family: "Raleway";
  src: url("fonts/Raleway-Regular.ttf") format("truetype");
  font-weight: 400;
}
@font-face {
  font-family: "swiper-icons";
  src: url("data:application/font-woff;charset=utf-8;base64, ") format("woff");
  font-weight: 400;
}
```

### Type Scale

| Role | Family | Size | Weight |
|------|--------|------|--------|
| Heading 1 | Noto Serif Japanese | 20px | 700 |
| Heading 2 | Noto Serif Japanese | 13px | 700 |
| Heading 3 | Noto Serif Japanese | 100% | 700 |
| Body | Raleway | 123% | 400 |
| Caption | Raleway | 85% | 400 |

### Typography Rules

- Body/UI: **Noto Serif Japanese**, Headings: **Raleway** — these are the only display fonts
- Max 3-4 font sizes per screen
- Headings: weight 600-700, body: weight 400
- Use color and opacity for text hierarchy, not additional font sizes
- Line height: 1.5 for body, 1.2 for headings

## Spacing & Layout

### Base Grid: 4px

Every dimension (margin, padding, gap, width, height) must be a multiple of **4px**.

### Spacing Scale

`2, 4, 6, 8, 10, 12, 14, 16, 18, 22, 24, 26` px

### Spacing as Meaning

| Spacing | Use |
|---------|-----|
| 4-8px | Tight: related items (icon + label, avatar + name) |
| 12-16px | Medium: between groups within a section |
| 24-32px | Wide: between distinct sections |
| 48px+ | Vast: major page section breaks |

### Border Radius

Scale: `.1em, .2em, .5em, .75em, 2.5em, 3em, 10px, 100%`
Default: `2.5em`

### Container

Max-width: `960px`, centered with auto margins.

### Breakpoints

| Name | Value |
|------|-------|
| md | 680px |
| md | 768px |

Mobile-first: design for small screens, layer on responsive overrides.

## Component Patterns

### Card

```css
.card {
  background: #000000;
  border-radius: 2.5em;
  padding: 16px;
  box-shadow: 0 0 .8em 0 rgba(0,0,0,0.5);
}
```

```html
<div class="card">
  <h3>Card Title</h3>
  <p>Card content goes here.</p>
</div>
```

### Button

```css
/* Primary */
.btn-primary {
  background: #cc0000;
  color: #ffffff;
  border-radius: 2.5em;
  padding: 8px 16px;
  font-weight: 500;
  transition: opacity 150ms ease;
}
.btn-primary:hover { opacity: 0.9; }

/* Ghost */
.btn-ghost {
  background: transparent;
  border: 1px solid #444444;
  color: #ffffff;
  border-radius: 2.5em;
  padding: 8px 16px;
}
```

```html
<button class="btn-primary">Get Started</button>
<button class="btn-ghost">Learn More</button>
```

### Input

```css
.input {
  background: #333333;
  border: 1px solid #444444;
  border-radius: 2.5em;
  padding: 8px 12px;
  color: #ffffff;
  font-size: 14px;
}
.input:focus { border-color: #cc0000; outline: none; }
```

```html
<input class="input" type="text" placeholder="Search..." />
```

### Badge / Chip

```css
.badge {
  display: inline-flex;
  align-items: center;
  padding: 4px 8px;
  border-radius: 9999px;
  font-size: 12px;
  font-weight: 500;
  background: #000000;
  color: #726660;
}
```

```html
<span class="badge">New</span>
<span class="badge">Beta</span>
```

### Modal / Dialog

```css
.modal-backdrop { background: rgba(0, 0, 0, 0.6); }
.modal {
  background: #000000;
  border-radius: 100%;
  padding: 24px;
  max-width: 480px;
  width: 90vw;
  box-shadow: 0 0 .8em 0 rgba(0,0,0,0.5);
}
```

```html
<div class="modal-backdrop">
  <div class="modal">
    <h2>Dialog Title</h2>
    <p>Dialog content.</p>
    <button class="btn-primary">Confirm</button>
    <button class="btn-ghost">Cancel</button>
  </div>
</div>
```

### Table

```css
.table { width: 100%; border-collapse: collapse; }
.table th {
  text-align: left;
  padding: 8px 12px;
  font-weight: 500;
  font-size: 12px;
  color: #726660;
  text-transform: uppercase;
  letter-spacing: 0.05em;
  border-bottom: 1px solid #444444;
}
.table td {
  padding: 12px;
  border-bottom: 1px solid #444444;
}
```

```html
<table class="table">
  <thead><tr><th>Name</th><th>Status</th><th>Date</th></tr></thead>
  <tbody>
    <tr><td>Item One</td><td>Active</td><td>Jan 1</td></tr>
    <tr><td>Item Two</td><td>Pending</td><td>Jan 2</td></tr>
  </tbody>
</table>
```

### Navigation

```css
.nav {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 12px 16px;
}
.nav-link {
  color: #726660;
  padding: 8px 12px;
  border-radius: 2.5em;
  transition: color 150ms;
}
.nav-link:hover { color: #ffffff; }
.nav-link.active { color: #cc0000; }
```

```html
<nav class="nav">
  <a href="/" class="nav-link active">Home</a>
  <a href="/about" class="nav-link">About</a>
  <a href="/pricing" class="nav-link">Pricing</a>
  <button class="btn-primary" style="margin-left: auto">Get Started</button>
</nav>
```

## Page Structure

The following page sections were detected:

- **Navigation** — Top navigation bar (2 items)
- **Hero** — Hero section (detected from heading structure)
- **Footer** — Page footer with links and info (13 items)

When building pages, follow this section order and structure.

## Animation & Motion

This project uses **subtle motion**. Transitions smooth state changes without calling attention.

### CSS Animations

- `zoom`
- `swiper-preloader-spin`
- `spin`

### Motion Tokens

- **Duration scale:** `200ms`, `300ms`, `400ms`, `1000ms`
- **Easing functions:** `ease-in-out`, `ease-out`

### Motion Guidelines

- **Duration:** Use values from the duration scale above. Short (200ms) for micro-interactions, long (1000ms) for page transitions
- **Easing:** Use `ease-in-out` as the default easing curve
- **Direction:** Elements enter from bottom/right, exit to top/left
- **Reduced motion:** Always respect `prefers-reduced-motion` — disable animations when set

## Depth & Elevation

### Shadow Tokens

- Subtle: `0 0 0 1px #CCC`
- Raised (cards, buttons): `0 0 .8em 0 rgba(0,0,0,0.5)`

### Z-Index Scale

`0, 1, 10, 50, 100, 101, 120, 200, 9998, 9999`

Use these exact values — never invent z-index values.

## Anti-Patterns (Never Do)

- **No blur effects** — no backdrop-blur, no filter: blur()
- **No zebra striping** — tables and lists use borders for separation
- **No invented colors** — every hex value must come from the palette above
- **No arbitrary spacing** — every dimension is a multiple of 4px
- **No extra fonts** — only Noto Serif Japanese and Raleway are allowed
- **No arbitrary border-radius** — use the scale: .1em, .2em, .5em, .75em, 2.5em, 3em, 10px
- **No opacity for disabled states** — use muted colors instead

## Workflow

1. **Read** `references/DESIGN.md` before writing any UI code
2. **Pick colors** from the Color System section — never invent new ones
3. **Set typography** — Noto Serif Japanese, Raleway only, using the type scale
4. **Build layout** on the 4px grid — check every margin, padding, gap
5. **Match components** to patterns above before creating new ones
6. **Apply elevation** — use shadow tokens
7. **Validate** — every value traces back to a design token. No magic numbers.

## Brand Spec

- **Favicon:** `/favicon.ico`
- **Site URL:** `https://kuraho.jp/`
- **Brand color:** `#cc0000`
- **Brand typeface:** Noto Serif Japanese

## Quick Reference

```
Background:     #333333
Surface:        #000000
Text:           #ffffff / #726660
Accent:         #cc0000
Border:         (not extracted)
Font:           Noto Serif Japanese
Spacing:        4px grid
Radius:         2.5em
Components:     5 detected
```

## When to Trigger

Activate this skill when:
- Creating new components, pages, or visual elements for kuraho
- Writing CSS, Tailwind classes, styled-components, or inline styles
- Building page layouts, templates, or responsive designs
- Reviewing UI code for design consistency
- The user mentions "kuraho" design, style, UI, or theme
- Generating mockups, wireframes, or visual prototypes

---

# Full Reference Files

> Every output file is embedded below. Claude has full design system context from /skills alone.

## Design System Tokens (DESIGN.md)

# kuraho DESIGN.md

> Auto-generated design system — reverse-engineered via static analysis by skillui.
> Frameworks: None detected
> Colors: 19 · Fonts: 2 · Components: 5
> Icon library: not detected · State: not detected
> Primary theme: dark · Dark mode toggle: no · Motion: subtle

## Visual Reference

**Match this design exactly** — study colors, fonts, spacing, and component shapes before writing any UI code.

![kuraho Homepage](../screenshots/homepage.png)

---

## 1. Visual Theme & Atmosphere

This is a **dark-themed** interface with a warm tone. Depth is expressed through layered shadows and subtle surface color variation. Typography pairs **Raleway** for display/headings with **Noto Serif Japanese** for body text, creating clear visual hierarchy through type contrast. Spacing follows a **4px base grid** (compact density), with scale: 2, 4, 6, 8, 10, 12, 14, 16px. The accent color **#cc0000** anchors interactive elements (buttons, links, focus rings). Motion is subtle — smooth transitions (150-300ms) ease state changes without drawing attention.

---

## 2. Color Palette & Roles

| Token | Hex | Role | Use |
|---|---|---|---|
| background | `#333333` | background | Page background, darkest surface |
| swiper-navigation-color | `#000000` | surface | Card and panel backgrounds |
| swiper-navigation-color | `#ffffff` | text-primary | Headings and body text |
| text-muted | `#726660` | text-muted | Captions, placeholders, secondary info |
| accent | `#cc0000` | accent | CTAs, links, focus rings, active states |
| danger | `#7a2c1d` | danger | Error states, destructive actions |
| warning | `#ff8500` | warning | Warning states, caution indicators |
| info | `#0066cc` | info | Informational highlights |
| unknown | `#e3e0df` | unknown | Palette color |
| unknown | `#cccccc` | unknown | Palette color |
| unknown | `#ffb90a` | unknown | Palette color |
| unknown | `#999999` | unknown | Palette color |
| unknown | `#7f001b` | unknown | Palette color |
| unknown | `#f1efef` | unknown | Palette color |
| unknown | `#990000` | unknown | Palette color |
| unknown | `#ff9933` | unknown | Palette color |
| unknown | `#ff6633` | unknown | Palette color |
| swiper-theme-color | `#007aff` | unknown | Palette color |
| unknown | `#a26b60` | unknown | Palette color |


---

## 3. Typography Rules

**Font Stack:**
- **Noto Serif Japanese** — Heading 1, Heading 2, Heading 3
- **Raleway** — Body, Caption

**Font Sources:**

```css
@font-face {
  font-family: "Noto Serif Japanese";
  src: url("fonts/NotoSerifJapanese-Regular.woff") format("woff");
  font-weight: 400;
}
@font-face {
  font-family: "Raleway";
  src: url("fonts/Raleway-Bold.ttf") format("truetype");
  font-weight: 700;
}
@font-face {
  font-family: "Raleway";
  src: url("fonts/Raleway-Regular.ttf") format("truetype");
  font-weight: 400;
}
@font-face {
  font-family: "swiper-icons";
  src: url("data:application/font-woff;charset=utf-8;base64, ") format("woff");
  font-weight: 400;
}
```

| Role | Font | Size | Weight |
|---|---|---|---|
| Heading 1 | Noto Serif Japanese | 20px | 700 |
| Heading 2 | Noto Serif Japanese | 13px | 700 |
| Heading 3 | Noto Serif Japanese | 100% | 700 |
| Body | Raleway | 123% | 400 |
| Caption | Raleway | 85% | 400 |

**Typographic Rules:**
- Limit to 2 font families max per screen
- Use **Noto Serif Japanese** for body/UI text, **Raleway** for display/headings
- Maintain consistent hierarchy: no more than 3-4 font sizes per screen
- Headings use bold (600-700), body uses regular (400)
- Line height: 1.5 for body text, 1.2 for headings
- Use color and opacity for secondary hierarchy, not additional font sizes


---

## 4. Component Stylings

### Layout (1)

**Footer** — `html`

### Navigation (1)

**Navigation** — `html`

### Data Input (2)

**Button** — `html`

**Input** — `html`
- State: :focus, :placeholder

### Media (1)

**Image** — `html`



---

## 5. Layout Principles

- **Base spacing unit:** 4px
- **Spacing scale:** 2, 4, 6, 8, 10, 12, 14, 16, 18, 22, 24, 26
- **Border radius:** .1em, .2em, .5em, .75em, 2.5em, 3em, 10px, 100%
- **Max content width:** 960px

**Spacing as Meaning:**
| Spacing | Use |
|---|---|
| 4-8px | Tight: related items within a group |
| 12-16px | Medium: between groups |
| 24-32px | Wide: between sections |
| 48px+ | Vast: major section breaks |


---

## 6. Depth & Elevation

### Flat — subtle depth hints

- `0 0 0 1px #CCC`

### Raised — cards, buttons, interactive elements

- `0 0 .8em 0 rgba(0,0,0,0.5)`

### Z-Index Scale

`0, 1, 10, 50, 100, 101, 120, 200, 9998, 9999`



---

## 7. Animation & Motion

This project uses **subtle motion**. Transitions smooth state changes without demanding attention.

### CSS Animations

- `@keyframes zoom`
- `@keyframes swiper-preloader-spin`
- `@keyframes spin`

### Motion Guidelines

- Duration: 150-300ms for micro-interactions, 300-500ms for page transitions
- Easing: `ease-out` for enters, `ease-in` for exits
- Always respect `prefers-reduced-motion`


---

## 8. Do's and Don'ts

### Do's

- Use `#cc0000` for interactive elements (buttons, links, focus rings)
- Use `#333333` as the primary page background
- Pair **Noto Serif Japanese** (body) with **Raleway** (display) — these are the only allowed fonts
- Follow the **4px** spacing grid for all margins, padding, and gaps
- Use the defined shadow tokens for elevation — see Section 6
- Use border-radius from the scale: .1em, .2em, .5em, .75em, 2.5em
- Reuse existing components from Section 4 before creating new ones

### Don'ts

- Don't introduce colors outside this palette — extend the design tokens first
- Don't introduce additional font families beyond Noto Serif Japanese and Raleway
- Don't use arbitrary spacing values — stick to multiples of 4px
- Don't create custom box-shadow values outside the system tokens
- Don't use arbitrary border-radius values — pick from the defined scale
- Don't duplicate component patterns — check Section 4 first
- Don't use backdrop-blur or blur effects

### Anti-Patterns (detected from codebase)

- No blur or backdrop-blur effects
- No zebra striping on tables/lists


---

## 9. Responsive Behavior

| Name | Value | Source |
|---|---|---|
| md | 680px | css |
| md | 768px | css |

**Approach:** Use `@media (min-width: ...)` queries matching the breakpoints above.


---

## 10. Agent Prompt Guide

Use these as starting points when building new UI:

### Build a Card

```
Background: #000000
Border: 1px solid var(--border)
Radius: 2.5em
Padding: 16px
Font: Noto Serif Japanese
Use shadow tokens from Section 6.
```

### Build a Button

```
Primary: bg #cc0000, text white
Ghost: bg transparent, border var(--border)
Padding: 8px 16px
Radius: 2.5em
Hover: opacity 0.9 or lighter shade
Focus: ring with #cc0000
```

### Build a Page Layout

```
Background: #333333
Max-width: 960px, centered
Grid: 4px base
Responsive: mobile-first, breakpoints from Section 9
```

### Build a Stats Card

```
Surface: #000000
Label: #726660 (muted, 12px, uppercase)
Value: #ffffff (primary, 24-32px, bold)
Status: use success/warning/danger from Section 2
```

### Build a Form

```
Input bg: #333333
Input border: 1px solid var(--border)
Focus: border-color #cc0000
Label: #726660 12px
Spacing: 16px between fields
Radius: 2.5em
```

### General Component

```
1. Read DESIGN.md Sections 2-6 for tokens
2. Colors: only from palette
3. Font: Noto Serif Japanese, type scale from Section 3
4. Spacing: 4px grid
5. Components: match patterns from Section 4
6. Elevation: shadow tokens
```

## Bundled Fonts (fonts/)

The following font files are bundled in the `fonts/` directory:

- `fonts/NotoSerifJapanese-Regular.woff`
- `fonts/Raleway-Black.ttf`
- `fonts/Raleway-Bold.ttf`
- `fonts/Raleway-ExtraBold.ttf`
- `fonts/Raleway-ExtraLight.ttf`
- `fonts/Raleway-Light.ttf`
- `fonts/Raleway-Medium.ttf`
- `fonts/Raleway-Regular.ttf`
- `fonts/Raleway-SemiBold.ttf`
- `fonts/Raleway-Thin.ttf`

Use these local font files in `@font-face` declarations instead of fetching from Google Fonts.

## Homepage Screenshots (screenshots/)

![homepage.png](screenshots/homepage.png)

