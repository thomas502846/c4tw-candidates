  ...
  return (
    <Gutter className="cms-guide">
      ...
      <section className="cms-guide__section">
        ...
        <p>
          內容支援繁體中文與英文兩種版本。要編哪一個版本，就用編輯頁面上方的「語言地區」切換器切過去——切到 English，欄位就會顯示英文版的內容，填好英文、發布即可。
        </p>
        <p>
          要注意兩件事是分開的：「語言地區」是切換你正在編輯的內容版本；後台選單、按鈕的語言則在「帳號設定」裡，是每個人自己的偏好。編輯英文內容時，不需要把後台介面也換成英文——介面維持中文、只把「語言地區」切到 English 即可。
        </p>
        <p>
          只有文字類欄位（標題、內文等）會分中／英兩份；圖片和共用設定兩個版本共用，所以通常編英文版時只需要改文字，不必重新上傳圖片。
        </p>
      </section>
      ...
      <section className="cms-guide__section">
        ...
        <p className="cms-guide__note">
          每個位置只要上傳一張圖就好。系統會自動為平板、手機產生較小的尺寸，不需要另外準備多張。建議「上傳大一點」比較清晰，系統不會因為檔案大而變慢。下表的「電腦／平板／手機顯示」是整張圖縮小呈現的大約寬度——數字變小代表圖變小，整張圖都會顯示，不會被切掉；數字為近似值，會依版面而略有不同。
        </p>
        ...
        <p className="cms-guide__note">
          只有一種情況圖會被「裁切」：滿版主視覺、背景大圖、卡片這類需要填滿固定形狀的圖。當手機把框變窄變高、框的形狀和圖不一樣時，系統會從畫面正中央往外保留、上下或左右切掉一樣多。所以這類圖請把重要的人或字放在正中央，邊緣多留一些空間。手機上實際會切成怎樣，可以在編輯頁右側的「即時預覽」切到手機尺寸直接看。
        </p>
      </section>
